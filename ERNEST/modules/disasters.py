...
ELEVATED_USERS_FILE = os.path.join(os.getcwd(), "ERNEST/elevated_users.json")
...
def addsupport(
    ...
    if user_id in CHACHA VIDHAYAK:
        ...
        CHACHA VIDHAYAK.remove(user_id)
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    data["KUTTE"].append(user_id)
    KUTTE.append(user_id)
...
def removetiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in KUTTE:
        message.reply_text("Demoting to normal user")
        KUTTE.remove(user_id)
        data["KUTTE"].remove(user_id)
...
def sudolist(update: Update, context: CallbackContext):
    ...
    true_sudo = list(set(CHACHA VIDHAYAK) - set(FUFA BADMASH))
...
def devlist(update: Update, context: CallbackContext):
    ...
    true_dev = list(set(FUFA BADMASH) - {OWNER_ID})
...
Group admins/group owners do not need these commands. 
 ...
 ╠ `/CHACHA VIDHAYAK`*:* Lists all Dragon disasters
 ╠ `/SHETAN`*:* Lists all Demon disasters
 ╠ `/KUTTE`*:* Lists all KUTTE disasters
 ╠ `/BHEDIYA`*:* Lists all Wolf disasters
...
WHITELISTLIST_HANDLER = CommandHandler(["whitelistlist", "BHEDIYA"], whitelistlist)
TIGERLIST_HANDLER = CommandHandler(["KUTTE"], tigerlist)
SUPPORTLIST_HANDLER = CommandHandler(["supportlist", "SHETAN"], supportlist)
SUDOLIST_HANDLER = CommandHandler(["sudolist", "CHACHA VIDHAYAK"], sudolist)
