...
import html
import requests
import ERNEST.modules.sql.kuki_sql as sql
...
from ERNEST.modules.helper_funcs.chat_status import user_admin
...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_kuki:
        ...
        msg.reply_text("ERNEST AI CHAT is now successfully enabled for this chat!")
        message = (
        ...
        return message
    msg.reply_text("ERNEST AI CHAT is already enabled for this chat!")
    return ""
...
def rem_chat(update: Update, context: CallbackContext):
    ...
    is_kuki = sql.is_kuki(chat.id)
    if not is_kuki:
        msg.reply_text("Hey, ERNEST AI CHAT isn't enabled yet!")
        return ""
    sql.rem_kuki(chat.id)
    msg.reply_text("SED, ERNEST AI CHAT is disabled successfully!")
    message = (
        f"<b>{html.escape(chat.title)}:</b>\n"
        f"AI_DISABLED\n"
        f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
__help__ = """
ChatBot utilizes the Branshop's API and allows ERNEST to talk to the users, reply to them and provides a more interactive and awesome group chat experience.
*Commands:*
*Admins only:*
